...
prepare_response_var <- function(dataset, response_var) {
  if (!response_var %in% names(dataset)) {
    stop(paste("Response variable", response_var, "not found in dataset."))
  }
...
prepare_predictor_matrix <- function(dataset, predictor_vars, intercept) {
  if (!all(predictor_vars %in% names(dataset))) {
    stop("Some predictor variables not found in the dataset.")
  }
...
fit_ols <- function(X, y) {
  ...
  betahat <- solve(XtX, Xty)  # Estimated coefficients
  y_hat <- X %*% betahat  # Predicted values

  residuals <- y - y_hat
  mse <- sum(residuals^2) / (nrow(X) - ncol(X))  # Mean squared error

  var_betahat <- solve(XtX) * mse  # Variance-covariance matrix
  se_betahat <- sqrt(diag(var_betahat))  # Standard errors for betahat
